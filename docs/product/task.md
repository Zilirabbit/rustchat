...
目标：让前端基础聊天页面完整接入现有群聊 HTTP API 与 WebSocket 群消息能力，完成“创建群聊 -> 添加成员 -> 群消息收发 -> 退出群聊 -> 刷新恢复”的演示闭环。

### 任务

- [x] 补齐前端群聊 API 封装：`POST /api/sessions/group`
- [x] 补齐前端添加群成员 API 封装：`POST /api/sessions/<id>/members`
- [x] 补齐前端退出群聊 API 封装：`DELETE /api/sessions/<id>/members/me`
- [x] 补齐前端群聊 request / response 类型定义
- [x] 在聊天页侧栏增加内联群聊面板
- [x] 支持填写群名、搜索用户、多选初始成员并创建群聊
- [x] 创建群聊成功后刷新会话列表并自动打开该群聊
- [x] 选中群聊后支持搜索并添加成员
- [x] 选中群聊后支持当前用户退出群聊
- [x] 退出当前群聊后清理当前选中态、历史消息缓存并刷新会话列表
...
- 用户可打开群聊并发送文本消息
- 在线群成员可收到群消息实时推送
- 群聊会出现在会话列表中，并展示最近消息、时间和未读数
- 用户可从当前选中的群聊中继续添加成员
- 用户可退出当前群聊，退出后该群聊从当前用户会话列表移除
- 刷新 `/chat` 后，若缓存的群聊仍属于当前用户，可恢复选中状态并加载历史消息

### 验收建议

- 执行 `cd frontend && npm run build`
- 执行 `cd backend && cargo test`
- 使用三个浏览器登录三个用户，验证 A 创建群聊并选择 B，A 添加 C，A/B/C 群内收发消息
- 验证 B/C 未打开群聊时 `unread_count` 增加，打开群聊后调用已读接口归零
- 验证成员退出群聊后，再次查询 `GET /api/conversations` 不再返回该群聊
...
- [x] 后端新增 `GET /api/sessions/{session_id}/members`
- [x] 仅群成员可查询当前群成员列表
- [x] 返回 `user_id`、`username`、`role`、`joined_at`
- [x] 前端补齐群成员列表 API 封装与类型定义
- [x] 打开群聊时展示当前成员列表
- [x] 添加成员成功后刷新成员列表

### 5.5.2 退出群聊后的状态一致性

- [x] 退出当前群聊后清空 `activeSessionId`
- [x] 删除该群聊历史消息缓存
- [x] 删除该群聊成员列表缓存
- [x] 清理本地保存的当前会话 ID
- [x] 重新拉取 `GET /api/conversations`
- [x] 退出后忽略该群聊残留实时消息，避免重新出现在会话列表

### 5.5.3 WebSocket 连接体验
...
### 5.5.3.1 WebSocket 连接质量增强

- [ ] 展示最近一次连接成功时间
- [ ] 展示最近一次断开时间
- [ ] 展示下一次重连倒计时
- [ ] 区分首次连接、自动重连、重连失败、用户主动离线等状态

### 5.5.3.2 WebSocket 心跳与超时检测

- [ ] 前端定期发送 `ping`
- [ ] 后端返回 `pong`
- [ ] 前端记录最近一次 `pong` 时间
- [ ] 超过阈值未收到 `pong` 时主动关闭连接并进入重连
- [ ] 浏览器 tab 不可见时降低心跳频率
- [ ] 浏览器 tab 恢复可见时立即检查连接状态

### 5.5.3.3 WebSocket 重连后数据补偿

- [x] 重连成功后重新拉取 `GET /api/conversations`
- [x] 重连成功后重新拉取当前会话历史消息
- [x] 重连成功后重新调用当前会话已读接口
- [x] 断线期间收到的新消息不应因会话列表未刷新而丢失展示
- [x] 重连补偿过程需要避免重复插入已存在消息

### 5.5.3.4 多窗口连接协调

- [ ] 使用 `BroadcastChannel` 同步登录、登出和连接状态
- [ ] 同一账号多 tab 打开时避免重复提示无意义的重连错误
- [ ] 一个 tab 登出时通知其他 tab 清理登录态与 WebSocket 连接
- [ ] tab 恢复前台时根据共享状态决定是否立即重连
...
- [x] 消息发送失败状态
- [x] 失败消息重试入口
- [x] 断线或重连中发送的消息进入本地 pending 队列
- [x] WebSocket 重连成功后自动补发 pending 消息
- [x] 补发成功后更新消息状态为已发送
- [x] 补发失败后保留失败状态并允许手动重试

完成说明：

- WebSocket `send_message` 支持可选 `client_message_id`，后端在 `message_sent / error` 中回传该 ID，用于前端精确匹配本地乐观消息。
- 前端发送消息会先插入本地消息，并展示 `等待重连 / 发送中 / 已发送 / 发送失败` 状态。
- 断线或重连中发送的消息仅进入当前页面内存 pending 队列；重连成功并完成现有数据补偿后自动补发。
- 发送失败或发送超时后保留失败状态，并提供手动重试入口。
- 暂不做 pending 队列持久化、跨 tab 同步、草稿、附件、撤回、编辑和已读回执；这些不属于本小节范围。
