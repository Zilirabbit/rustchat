...
fn extract_file_meta(content: &str) -> (String, Option<i64>, Option<String>, Option<i64>, Option<String>) {
    match serde_json::from_str::<serde_json::Value>(content) {
        Ok(json) => {
            let file_name = json.get("file_name").and_then(|v| v.as_str()).unwrap_or("file").to_string();
            let file_id = json.get("file_id").and_then(|v| v.as_i64());
            let file_size = json.get("file_size").and_then(|v| v.as_i64());
            let file_type = json.get("file_type").and_then(|v| v.as_str()).map(|s| s.to_string());
            (file_name.clone(), file_id, Some(file_name), file_size, file_type)
        }
        Err(_) => {
            // Fallback: treat content as the filename
            (content.to_string(), None, Some(content.to_string()), None, None)
        }
    }
}
