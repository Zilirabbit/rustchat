    ...
    R: FileRepository,
{
    ...
    pub async fn init_upload(
        ...
        if file_name.trim().is_empty() {
            return Err(AppError::BadRequest("file name cannot be blank".to_string()));
        }
        if total_chunks == 0 {
            return Err(AppError::BadRequest("total_chunks must be positive".to_string()));
        }

        // Verify session membership
        if !self.repo.is_session_member(session_id, user_id).await? {
            return Err(AppError::Forbidden("you are not a member of this session".to_string()));
        }
        ...
        fs::create_dir_all(&temp_dir).await.map_err(|e| {
            AppError::internal(anyhow::anyhow!("failed to create temp dir: {}", e))
        })?;
    ...
    pub async fn save_chunk(
        ...
        file.flush().await.map_err(|e| {
            AppError::internal(anyhow::anyhow!("failed to flush chunk: {}", e))
        })?;
    ...
    pub async fn complete_upload(
        ...
        let final_filename = format!("{}_{}", chrono::Utc::now().format("%Y%m%d%H%M%S"), sanitize_filename(&pending.file_name));
        ...
        fs::rename(&reassembled_path, &final_path).await.map_err(|e| {
            AppError::internal(anyhow::anyhow!("failed to move file: {}", e))
        })?;
    ...
    pub async fn verify_file_access(&self, user_id: i64, file_id: i64) -> AppResult<FileRecord> {
        ...
        if !self.repo.is_session_member(file.session_id, user_id).await? {
            return Err(AppError::Forbidden("you do not have access to this file".to_string()));
        }
    ...
    pub fn start_cleanup_task(self) where Self: 'static + Send {
...
fn sanitize_filename(name: &str) -> String {
    ...
    if sanitized.len() > 128 {
        if let Some(ext) = std::path::Path::new(&sanitized).extension().and_then(|e| e.to_str()) {
