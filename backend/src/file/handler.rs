...
pub async fn init_upload(
    ...
    let file_service = state.file_service.as_ref().ok_or(AppError::DbNotConfigured)?;
...
pub async fn upload_chunk(
    ...
    if body.is_empty() {
        return Err(AppError::BadRequest("chunk data cannot be empty".to_string()));
    }
...
pub async fn complete_upload(
    ...
    let file_name = file_meta["file_name"].as_str().unwrap_or("file").to_string();
    let file_size = file_meta["file_size"].as_i64().unwrap_or(0);
    let file_type = file_meta["file_type"].as_str().unwrap_or("application/octet-stream").to_string();
...
pub async fn download_file(
    ...
    let data = tokio::fs::read(&full_path).await.map_err(|_| {
        AppError::NotFound("file not found on storage".to_string())
    })?;
